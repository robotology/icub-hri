...
public:
    ...
    */
    ...
    /**
    * The string labelling the kinect node (of a human kinematic structure, one of the ICUBHRI_OPC_BODY_PART_TYPE_XXX) that corresponds to the iCub's joint
    */
    std::string m_kinectNode;

    virtual bool isType(std::string _entityType) const
    {
        if (_entityType == ICUBHRI_OPC_ENTITY_BODYPART)
            return true;
        else
