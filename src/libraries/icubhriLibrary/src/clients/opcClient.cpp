...
Entity *OPCClient::getEntity(int id, bool forceUpdate)
{
    ...
    if(newEntityType == "entity")
        newE = new Entity();
    else if (newEntityType == ICUBHRI_OPC_ENTITY_OBJECT)
        newE = new Object();
    else if (newEntityType == ICUBHRI_OPC_ENTITY_AGENT)
        newE = new Agent();
    else if (newEntityType == ICUBHRI_OPC_ENTITY_ACTION)
        newE = new Action();
        ...
        Entity* complement_manner)
{
    ...
    sub.addString(ICUBHRI_OPC_ENTITY_TAG);
    sub.addString("==");
    sub.addString(ICUBHRI_OPC_ENTITY_RELATION);
    query.addList() = sub;
...
std::list<std::shared_ptr<Entity>> OPCClient::EntitiesCacheCopy() const
{
    ...
    for(const auto& it : this->entitiesByID)
    {
        ...
        if (it.second->m_entity_type == ICUBHRI_OPC_ENTITY_AGENT)
        {
            E = std::shared_ptr<Agent>(new Agent());
        }
        else if (it.second->m_entity_type == ICUBHRI_OPC_ENTITY_OBJECT)
        {
            E = std::shared_ptr<Object>(new Object());
        }
        else if (it.second->m_entity_type == ICUBHRI_OPC_ENTITY_ACTION)
        {
