...
bool icubhri::SubSystem_emotion::connect() {
...
icubhri::SubSystem_emotion::SubSystem_emotion(const std::string &masterName) : SubSystem(masterName) {
    portRPC.open(("/" + m_masterName + "/emotion:rpc").c_str());
    m_type = SUBSYSTEM_EMOTION;
}

void icubhri::SubSystem_emotion::Close() {
    portRPC.interrupt();
    portRPC.close();
}

bool icubhri::SubSystem_emotion::setEmotion(std::string emotion, std::string part) {
...
bool icubhri::SubSystem_emotion::sendEmotion(std::pair<std::string, std::string> emPair) {
...
std::pair<std::string, std::string> icubhri::SubSystem_emotion::getEmotion()
{
