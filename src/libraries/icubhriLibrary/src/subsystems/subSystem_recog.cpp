...
bool icubhri::SubSystem_Recog::connect() {
...
icubhri::SubSystem_Recog::SubSystem_Recog(const std::string &masterName) : SubSystem(masterName){
    portRPC.open(("/" + m_masterName + "/recog:rpc").c_str());
    ears_port.open("/" + m_masterName + "/ears:o");
    m_type = SUBSYSTEM_RECOG;
}

void icubhri::SubSystem_Recog::Close() {
    portRPC.interrupt();
    portRPC.close();
    ears_port.interrupt();
    ears_port.close();
...
void icubhri::SubSystem_Recog::enableEars(bool on) {
    if (!yarp::os::Network::isConnected(ears_port.getName(), "/ears/rpc")){
...
void icubhri::SubSystem_Recog::waitForEars() {
    if (!yarp::os::Network::isConnected(ears_port.getName(), "/ears/rpc")){
        yarp::os::Network::connect(ears_port.getName(), "/ears/rpc");
    }
    if (yarp::os::Network::isConnected(ears_port.getName(), "/ears/rpc")) {
...
bool icubhri::SubSystem_Recog::interruptSpeechRecognizer() {
...
yarp::os::Bottle icubhri::SubSystem_Recog::recogFromGrammar(std::string &sInput)
{
    if (!yarp::os::Network::isConnected(portRPC.getName(), "/speechRecognizer/rpc")){
...
yarp::os::Bottle icubhri::SubSystem_Recog::recogFromGrammarLoop(std::string sInput, int iLoop, bool keepEarsEnabled, bool keepEarsDisabledAfterRecog)
{
    if (!yarp::os::Network::isConnected(portRPC.getName(), "/speechRecognizer/rpc")){
        if (!yarp::os::Network::connect(portRPC.getName(), "/speechRecognizer/rpc")){
            yarp::os::Bottle bReply;
