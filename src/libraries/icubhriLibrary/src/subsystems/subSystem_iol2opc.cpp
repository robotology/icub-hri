...
bool icubhri::SubSystem_IOL2OPC::connect() {
...
icubhri::SubSystem_IOL2OPC::SubSystem_IOL2OPC(const std::string &masterName) : SubSystem(masterName) {
    portRPC.open(("/" + m_masterName + "/iol2opc:rpc").c_str());
    m_type = SUBSYSTEM_IOL2OPC;
}

void icubhri::SubSystem_IOL2OPC::Close() {
    portRPC.interrupt();
    portRPC.close();
}

bool icubhri::SubSystem_IOL2OPC::changeName(const std::string &old_name, const std::string &new_name) {
...
void icubhri::SubSystem_IOL2OPC::pause() {
    yarp::os::Bottle bReq, bResp;
    bReq.addString("pause");
    portRPC.write(bReq, bResp);
}

void icubhri::SubSystem_IOL2OPC::resume() {
    yarp::os::Bottle bReq, bResp;
