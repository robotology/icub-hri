...
icubhri::SubSystem_Speech::SubSystem_Speech(const std::string &masterName) :SubSystem(masterName)
{
...
bool icubhri::SubSystem_Speech::connect()
{
...
void icubhri::SubSystem_Speech::TTS(const std::string &text, bool shouldWait) {
    if(text=="") {
...
void icubhri::SubSystem_Speech::SetOptions(const std::string &custom) {
    if(custom!="iCub") {
...
bool icubhri::SubSystem_Speech::isSpeaking() {
    yarp::os::Bottle cmd, reply;
    cmd.addVocab(VOCAB('s', 't', 'a', 't'));
    portRPC.write(cmd, reply);
...
void icubhri::SubSystem_Speech::Close()
{
    tts.interrupt();
    tts.close();
    portRPC.interrupt();
