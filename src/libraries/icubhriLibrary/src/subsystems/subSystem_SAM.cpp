...
bool icubhri::SubSystem_SAM::connect() {
...
icubhri::SubSystem_SAM::SubSystem_SAM(const std::string &masterName) : SubSystem(masterName) {
    portRPC.open(("/" + m_masterName + "/sam:rpc").c_str());
    m_type = SUBSYSTEM_SAM;
}

void icubhri::SubSystem_SAM::Close() {
    portRPC.interrupt();
    portRPC.close();
}

bool icubhri::SubSystem_SAM::attentionModulation(const std::string &mode) {
...
std::tuple<bool, std::string> icubhri::SubSystem_SAM::askXLabel(const std::string &model) {
    yarp::os::Bottle bReq, bResp;
