...
if __name__ == '__main__':
    ...
    logger = yarp.Log()
    iCub = icubclient.ICubClient("KARMAiCubClientExample", "icubClient", "example_ARE_KARMA.ini")

    # we connect to ARE and KARMA, which is defined in "example_ARE_KARMA.ini"
    if not iCub.connectSubSystems():
        logger.error("[KARMAiCubClientExample] KARMA seems unavailabe!")
    ...
    x[1] = x[1] - 0.1
    logger.info() << "[KARMAiCubClientExample] try to push left with KARMA..."
    ...
    iCub.home() # Home by using ARE
    logger.info() << "[KARMAiCubClientExample] try to push right with KARMA..."
    ...
    iCub.home() # Home by using ARE
    x[0] = x[0] - 0.15
    logger.info() << "[KARMAiCubClientExample] try to push front with KARMA..."
    ...
    x[1] =  0.1
    x[2] = -0.05
    logger.info() << "[KARMAiCubClientExample] try to pull with KARMA..."
    ...
    logger.info() << "[KARMAiCubClientExample] shutting down ... "
    iCub.close()
    yarp.Network.fini()